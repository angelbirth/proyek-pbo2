...
public class DialogEdit extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public DialogEdit(JFrame parent) {
		super(parent, "Edit Data", true);
		MainForm main = (MainForm) parent;
		this.parent = parent;
		setResizable(false);
		setBounds(100, 100, 530, 307);
		...
		{ // txt ID
			txtID = new JTextField(main.dataTable.getValueAt(
					main.dataTable.getSelectedRow(), 0).toString());
		...
		{
			txtPhone = new JTextField(main.dataTable.getValueAt(
					main.dataTable.getSelectedRow(), 6).toString());
		...
		{
			txtName = new JTextField(main.dataTable.getValueAt(
					main.dataTable.getSelectedRow(), 1).toString());
		...
		{
			txtEmail = new JTextField(main.dataTable.getValueAt(
					main.dataTable.getSelectedRow(), 7).toString());
		...
		}
		{
			...
			contentPanel.add(panel, gbc_panel);
			{
				...
				rdoMale.setSelected(main.dataTable
						.getValueAt(main.dataTable.getSelectedRow(), 2)
						.toString().equalsIgnoreCase("M") ? true : false);
				rdoMale.addActionListener(radioClick);
				panel.add(rdoMale);
			}
			{
				rdoFemale = new JRadioButton("Female");
				rdoFemale.setSelected(main.dataTable
						.getValueAt(main.dataTable.getSelectedRow(), 2)
						.toString().equalsIgnoreCase("F") ? true : false);
		...
		{
			txtJob = new JTextField(main.dataTable.getValueAt(
					main.dataTable.getSelectedRow(), 8).toString());
		...
		{
			txtBirthplace = new JTextField(main.dataTable.getValueAt(
					main.dataTable.getSelectedRow(), 3).toString());
		...
		{
			txtCompany = new JTextField(main.dataTable.getValueAt(
					main.dataTable.getSelectedRow(), 9).toString());
		...
		}
		{
			...
			dateChooser.setDate((Date) main.dataTable.getValueAt(
					main.dataTable.getSelectedRow(), 4));
		...
		{
			txtAddress = new JTextArea(main.dataTable.getValueAt(
					main.dataTable.getSelectedRow(), 5).toString());
	...
	private void btnOK_Click() {
		...
		try {
			...
			JOptionPane.showMessageDialog(DialogEdit.this,
	...
	private void editData() throws SQLException {
		if (((MainForm) parent).conn != null
				&& !((MainForm) parent).conn.isClosed()) {
			java.sql.PreparedStatement s = ((MainForm) parent).conn
					.prepareStatement("update phonebook set id=?,"
							+ "name=?,sex=?,birthplace=?,"
			...
			s.setObject(11, ((MainForm) parent).dataTable.getValueAt(
					((MainForm) parent).dataTable.getSelectedRow(), 0));
			s.execute();
			s.close();
